...
**FREERADIUS3_SECRET** же нужен для обращения к RADIUS серверу, но скрипт уже всё подставил в нужное место. Вам только остаётся прописать:

`reboot`

После перезагрузки WiFi будет настроен в режиме wpa2-enterprise с единственной учётной записью root с паролем FREERADIUS3_ROOT_PASSWORD. Можете протестировать, присоединившись к сети.