...
Если у вас есть интернет, рекомендуется сделать:

`opkg update`  - аналог "apt update" в ubuntu

На процесс установки это не повлияет никак, но если вы захотите установить определённые пакеты из сети, без этой команды они просто не проиндексируются.

Клонируем или скачиваем этот репозиторий (ветка master)
...
`cd USB/OpenWRT-for-Xiaomi-Mi-WiFi-router-Pro-r3p-with-FreeRADIUS-SQLite-and-SQLite-Web-Admin; bash setup.bash`
