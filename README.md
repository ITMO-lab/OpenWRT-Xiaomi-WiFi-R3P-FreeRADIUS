# Установка OpenWRT на роутер Xiaomi R3P [ Xiaomi Mi WiFi router Pro(r3p) ] с настройкой WiFi wpa2-enterprise с запущенными на роутере FreeRADIUS (для предоставления индивидуальных ключей wpa2-enterprise), SQLite (для хранения этих ключей) и SQLite Web Admin (для удобного редактирования учётных записей пользователей)
...
[**6 Установка FreeRADIUS**](#6-установка-freeradius)
...
![img](https://github.com/ITMO-lab/OpenWRT-for-Xiaomi-Mi-WiFi-router-Pro-r3p-with-FreeRADIUS-SQLite-and-SQLite-Web-Admin/blob/images/screenshots/5/13.png)
...
![img](https://github.com/ITMO-lab/OpenWRT-for-Xiaomi-Mi-WiFi-router-Pro-r3p-with-FreeRADIUS-SQLite-and-SQLite-Web-Admin/blob/images/screenshots/5/14.png)
...
# 6 Установка FreeRADIUS

**Далее важно понимать, есть ли у нас интернет или нет. Да, он настроен, но поблизости может просто не быть кабеля с интернетом, поэтому установку пакетов решено сделать максимально удобной.**
...
Клонируем или скачиваем этот репозиторий (ветка master) на своём компьютере.

`git clone https://github.com/ITMO-lab/OpenWRT-for-Xiaomi-Mi-WiFi-router-Pro-r3p-with-FreeRADIUS-SQLite-and-SQLite-Web-Admin.git`

Переносим на флешку в корень. Желательно, чтобы флешка была с файловой системой fat-32, но моя ntfs тоже смонтировалась, пусть и поругалась немного. Как же работать с флешкой? Очень просто - вставляем в единственный usb порт и пишем *продуманную* команду:
...
**root**	-	**FREERADIUS3_ROOT_PASSWORD** (можно поменять или добавить дефолтные учётные записи в `/etc/freeradius3/mods-config/files/authorize`)
...
# 7 Установка SQLite и PHP-web-Admin
